...
export async function validateAndNominate(user_nominator: { userId: number }, nominated_user_address: string) {
    ...
    const [
        ...
        supabase.from('app_user').select('max_nominations').eq('id', user_nominator.userId).single()
    ]);
    ...
    return await supabase
        ...
        .insert({ user_id_from: user_nominator.userId, user_id_nominated: nominated_user[0].id })
        .select();
}
