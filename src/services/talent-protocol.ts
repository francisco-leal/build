...
export async function searchTalentProtocolUser(
    ...
    if (querySearch.length === 42 && querySearch.startsWith('0x')) {
        ...
        try {
            const response = await fetch(`${api_url}/api/v2/passports/${walletAddress}`, {
                method: 'GET',
                headers: {
    ...
    try {
        const response = await fetch(`${api_url}/api/v2/passports?keyword=${querySearch}&page=1&per_page=10`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': api_token || ''
